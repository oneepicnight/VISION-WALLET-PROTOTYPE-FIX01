...
export default function Settings() {
  ...
  const handleExportBackup = async () => {
    ...
    try {
      ...
      if (!keystore) {
        alert('Could not decrypt keystore. You may need to re-create your wallet.')
        return
      }
  ...
  const handleWipeWallet = async () => {
    if (wipeConfirm !== 'VISION') {
      alert('Type "VISION" to confirm wallet wipe.')
      return
    }
    ...
    try {
      ...
      alert('Wallet wiped successfully. Redirecting...')
      window.location.href = '/'
    } catch (error) {
      console.error('Wipe failed:', error)
      alert('Failed to wipe wallet. Please try again.')
    }
  }
